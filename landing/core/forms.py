...
def inviter_email_validator(inviter_email):
    if inviter_email and not models.Associate.objects.filter(user__email=inviter_email).exists():
...
class SignupForm(forms.Form):
    ...
    date_of_birth = forms.DateField(label='Data de Nascimento', input_formats=['%d/%m/%Y'])
    ...
    email = forms.EmailField(label='E-mail', max_length=255, validators=[associate_email_validator])

    username = forms.CharField(label='Usuário', max_length=32)
    password = forms.CharField(label='Senha', max_length=100)

    inviter_email = forms.EmailField(label='E-mail de quem te convidou', required=False, max_length=255,
